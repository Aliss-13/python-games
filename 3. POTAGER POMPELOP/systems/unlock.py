def verifier_deblocage(state):
    ...
    if state.joueur.niveau >= 3 and 3 not in state.graines_disponibles:
        state.graines_disponibles.add(3)
        events.append({"type": "unlock_seed", "seed": 3})

    if state.joueur.niveau >= 4 and 4 not in state.graines_disponibles:
        state.graines_disponibles.add(4)
        events.append({"type": "unlock_seed", "seed": 4})

    if state.joueur.niveau >= 5 and 5 not in state.graines_disponibles:
